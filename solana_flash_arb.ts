...
function readConfig(): Config {
  ...
  return {
    ...
    slippageBps: envInt("SOL_FLASH_ARB_SLIPPAGE_BPS", 1, 0),
...
export function buildStableSwapInstruction(
  ...
  const [inputPool] = PublicKey.findProgramAddressSync(
    [Buffer.from("pool"), USDT_MINT.toBuffer()],
    STABLE_PROGRAM_ID,
  );
  ...
  const userInputAta = getAssociatedTokenAddressSync(USDT_MINT, wallet);
  const userOutputAta = getAssociatedTokenAddressSync(USDC_MINT, wallet);
...
async function getCapacitySizedCycle(
  ...
  for (
    ...
    if (capacityRaw <= config.stableCapacityBufferRaw) {
      throw new Error(
        `Stable.com capacity ${formatRaw(capacityRaw)} USDT is not above the configured ${formatRaw(config.stableCapacityBufferRaw)} USDT safety buffer`,
      );
    }
...
async function assertTokenAccountsExist(
  ...
  const usdtAta = getAssociatedTokenAddressSync(USDT_MINT, owner);
  ...
  if (!infos[1]) missing.push(`USDT ATA ${usdtAta.toBase58()}`);
  if (missing.length) {
    throw new Error(
...
async function main(): Promise<void> {
  ...
  console.log(
    `Stable.com capacity: ${formatRaw(sized.capacityRaw)} USDT (${formatRaw(sized.usableCapacityRaw)} usable)`,
  );
  console.log(
    `Jupiter guaranteed minimum: ${formatRaw(cycle.firstLegMinimumRaw)} USDT`,
  );
  console.log(
    `Stable.com executable output: ${formatRaw(cycle.secondLegMinimumRaw)} USDC`,
  );
  ...
  await assertTokenAccountsExist(connection, walletAddress);
