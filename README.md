...
The scanner also refuses every new first leg while the wallet holds more than the normal 0.1-token intermediate tolerance in USDG or PYUSD. When an exit fails, it writes one durable recovery plan containing the exact acquired token amount. The separately supervised `recovery_worker.py` watches only that amount on Jupiter and returns it to USDC only when the conservative net recovery profit exceeds `$0.50`. It includes the configured execution-cost reserve and a one-basis-point slippage reserve by default. The worker uses fresh WebSocket balance revisions for the return confirmation, with one RPC snapshot fallback; it never opens a first leg, never increases the planned amount to match a larger wallet balance, and leaves a manual-review lock if the planned balance changes unexpectedly. The scanner remains locked until the recovery is confirmed.
