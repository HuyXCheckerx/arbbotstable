#!/usr/bin/env python3
"""Compile, estimate, and optionally deploy the Ethereum executor from .env."""
...
SOURCE = PROJECT_DIR / "contracts" / "MorphoMatchaStableArb.sol"
SOLC_VERSION = "0.8.30"
EXPECTED_CHAIN_ID = 1
...
def compile_contract() -> tuple[list[dict], str]:
    with TemporaryDirectory(prefix="morpho-executor-solc-") as output_dir:
        command = [
            "npx",
            "--yes",
            f"solc@{SOLC_VERSION}",
        ...
        artifact_prefix = "contracts_MorphoMatchaStableArb_sol_MorphoMatchaStableArb"
